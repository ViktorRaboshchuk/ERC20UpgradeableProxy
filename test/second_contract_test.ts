...
describe("ERC20TokenV2", function () {
  ...
  beforeEach(async () => {
    ...
    const ERC20Token = await hre.ethers.getContractFactory("ERC20TokenV2", {signer: owner});
  ...
  describe("Check contract constructor", () => {
    it("Should mint 0 amount of tokens", async function () {
      ...
      expect(await balance).to.equal(BigInt(1000*10**18));
    });
    ...
    it("Check contract total supply", async function () {
      expect(await erc20Token.totalSupply()).to.equal(BigInt(1000*10**18));
    });
